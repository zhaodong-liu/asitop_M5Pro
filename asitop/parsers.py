...
def parse_cpu_metrics(powermetrics_parse):
    soc_name = os.popen('sysctl -a | grep machdep.cpu').read()
    if "Apple M5 Pro" in soc_name:
        p_core = []
        s_core = []
        ...
        for cluster in cpu_clusters:
            ...
            cpu_metric_dict[name+"_active"] = int((1 - cluster["idle_ratio"])*100)
            for cpu in cluster["cpus"]:
                name = 'E-Cluster' if name.startswith("E") else 'P-Cluster'
                core = e_core if name.startswith("E") else p_core
                core.append(cpu["cpu"])
                cpu_metric_dict[name + str(cpu["cpu"]) + "_freq_Mhz"] = int(cpu["freq_hz"] / (1e6))
                cpu_metric_dict[name + str(cpu["cpu"]) + "_active"] = int((1 - cpu["idle_ratio"]) * 100)
        cpu_metric_dict["p_core"] = p_core
        ...
        if "E-Cluster_active" not in cpu_metric_dict:
            e_active_vals = [v for k, v in cpu_metric_dict.items() if k.startswith("E") and k.endswith("_active")]
        ...
        if "E-Cluster_freq_Mhz" not in cpu_metric_dict:
            e_freq_vals = [v for k, v in cpu_metric_dict.items() if k.startswith("E") and k.endswith("_freq_Mhz")]
